...
setup_systemd_service() {
    ...
    if [ -z "$gost_path" ]; then
        if [ -f "/usr/local/bin/gost" ]; then gost_path="/usr/local/bin/gost";
        elif [ -f "/usr/bin/gost" ]; then gost_path="/usr/bin/gost";
        else
    ...
    # 1. 创建服务模板文件 (类似 wg-quick@.service)
    local service_template="/etc/systemd/system/gost@.service"
    if [ ! -f "$service_template" ]; then
        print_info "配置 systemd 服务模板 (gost@.service)..."
...
[Service]
Type=simple
ExecStart=$gost_path -C $CONFIG_DIR/config_%i/gost-config.json
Restart=always
RestartSec=5
LimitNOFILE=65535
...
EOF
    ...
    # 2. 停掉目前可能正在运行的手动进程 (清理旧的 nohup 进程)
    print_info "正在清理 ID $config_id 可能存在的后台进程..."
    stop_proxy_by_id "$config_id" >/dev/null 2>&1 || true

    # 3. 启用并开始服务
    print_info "正在通过 systemctl 启用并启动 gost@$config_id 服务..."
    systemctl enable "gost@$config_id" --now

    if systemctl is-active --quiet "gost@$config_id"; then
        print_success "配置 ID $config_id 已成功开启自启服务模式"
        print_info "您现在可以使用以下命令进行独立控制:"
        print_info "  systemctl status gost@$config_id"
        print_info "  systemctl restart gost@$config_id"
...
select_config_to_fortify() {
    ...
    echo ""
    print_info "请输入要开启自启服务的配置ID，或输入 'q' 退出:"
    while true; do
        read -p "配置ID: " input
...
show_main_menu() {
    ...
    echo "4) 查看所有配置"
    echo "5) 删除指定配置"
    echo "6) 将指定配置开启自启服务 (类似 wg-quick@id)"
    echo "7) 显示网络接口信息"
    echo "8) 退出"
    echo ""
}
...
show_usage() {
    ...
    echo "  -f, --fortify-id ID     将指定ID配置为独立自启服务"
