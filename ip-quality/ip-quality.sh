...
"Parameters 参数运行: bash [-4] [-6] [-f] [-h] [-j] [-i iface] [-l language] [-n] [-o output] [-x proxy] [-y] [-E]"
"            -4                             Test IPv4                                  测试IPv4"
"            -6                             Test IPv6                                  测试IPv6"
...
function MediaUnlockTest_Instagram(){
local temp_info="$Font_Cyan$Font_B${sinfo[media]}${Font_I}Instagram $Font_Suffix"
((ibar_step+=3))
show_progress_bar "$temp_info" $((40-10-${sinfo[lmedia]}))&
bar_pid="$!"&&disown "$bar_pid"
trap "kill_progress_bar" RETURN
instagram=()
local checkunlockurl="www.instagram.com"
local result1=$(Check_DNS_1 $checkunlockurl)
local result3=$(Check_DNS_3 $checkunlockurl)
local resultunlocktype=$(Get_Unlock_Type $result1 $result3)
local IG_HTML=$(curl $CurlARG -$1 -s --max-time 10 -H "User-Agent: $UA_Browser" "https://www.instagram.com/")
if [[ $IG_HTML == "curl"* ]];then
instagram[ustatus]="${smedia[bad]}"
...
return
elif [ "$IG_COUNTRY" == "CN" ]; then
instagram[ustatus]="${smedia[cn]}"
instagram[uregion]="  $Font_Red[CN]$Font_Green   "
instagram[utype]="${smedia[nodata]}"
return
...
echo -ne "\r$Font_Cyan${smedia[meida]}$Font_I TikTok   Netflix Youtube  AmazonPV  Instagram ChatGPT $Font_Suffix\n"
echo -ne "\r$Font_Cyan${smedia[status]}${tiktok[ustatus]}${netflix[ustatus]}${youtube[ustatus]}${amazon[ustatus]}${instagram[ustatus]}${chatgpt[ustatus]}$Font_Suffix\n"
